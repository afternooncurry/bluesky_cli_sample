...
ACTOR=`util/resolve_did.sh $1 $2`

# Get a list of user's feeds
LIMIT=50
FEED_URL="https://$SERVICE_ENDPOINT/xrpc/app.bsky.feed.getActorFeeds"
curl -G \
    -H "Authorization: Bearer ${API_KEY}" \
    --data-urlencode "actor=$ACTOR" \
    --data-urlencode "limit=$LIMIT" \
    "$FEED_URL" | jq ".feeds[]|{displayName,uri,actor:\"$ACTOR\",feedName:.uri,did}" |\
